--https://github.com/nus-mornin-lab/oxygenation_kc/blob/master/data-extraction/eICU/eicu_final_patient_results.sql
--eicu_final_patient_results.sql
CREATE TABLE eicu_final_patient_results AS (
...
sofa_results AS (
SELECT * FROM eicu.sofa_results),


icd_code AS (
SELECT
diag.patientunitstayid,
CAST(SUBSTR(diag.icd9code, 0, 3) as INT8) AS icd9code,
...
FROM eicu.careplaneol
WHERE activeupondischarge = 'TRUE'

UNION DISTINCT

SELECT DISTINCT patientunitstayid
FROM eicu.careplangeneral
...
, ce AS (
  SELECT DISTINCT 
    ...
    , CAST(chart.nursingchartvalue as FLOAT64) as spO2_Value
    , chart.nursingchartoffset AS charttime
  FROM eicu.nursecharting AS chart
  ...
  WHERE chart.nursingchartcelltypevalname = "O2 Saturation"
    ...
    AND CAST(chart.nursingchartvalue as FLOAT64) >= 10
    AND CAST(chart.nursingchartvalue as FLOAT64) <= 100
)
...
, SpO2 AS(
  SELECT DISTINCT
    ...
    , AVG(CAST(ce.spO2_Value < 94 AS INT8)) OVER(PARTITION BY ce.icustay_id) AS propBelow
    , AVG(CAST(ce.spO2_Value > 98 AS INT8)) OVER(PARTITION BY ce.icustay_id) AS propAbove
  FROM ce
)
...
, SpO2_24 AS (
  SELECT DISTINCT
    ...
    , AVG(CAST(ce.spO2_Value >= 94 AND ce.spO2_Value <= 98 AS INT8)) OVER(PARTITION BY ce.icustay_id) AS prop_24
  FROM ce
...
, SpO2_48 AS (
  SELECT DISTINCT
    ...
    , AVG(CAST(ce.spO2_Value >= 94 AND ce.spO2_Value <= 98 AS INT8)) OVER(PARTITION BY ce.icustay_id) AS prop_48
  FROM ce
    INNER JOIN oxygen_therapy ON ce.icustay_id = oxygen_therapy.icustay_id
  WHERE
...
, SpO2_72 AS (
  SELECT DISTINCT
    ...
    , AVG(CAST(ce.spO2_Value >= 94 AND ce.spO2_Value <= 98 AS INT8)) OVER(PARTITION BY ce.icustay_id) AS prop_72
  FROM ce
    INNER JOIN oxygen_therapy ON ce.icustay_id = oxygen_therapy.icustay_id
  WHERE
...
SELECT 
  ...
  CAST(pat.age AS FLOAT64) AS age,
